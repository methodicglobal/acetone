...
namespace Methodic.Acetone.IntegrationTests
{
    public class ServiceFabricTestClusterManager : IDisposable
    {
        ...
        public async Task<List<string>> DeploySolutionApplicationsAsync()
        {
            ...
            if (!IsClusterAvailable) return results;
            string root = TryLocateSolutionRoot();
            if (root == null) return results;
            var apps = new List<(string app, string service)>
            {
                ("MockApplication", "MockApi"),
                ("MockSystem", "MockService")
            };
            string imageStoreConnection = await GetImageStoreConnectionStringAsync();
            foreach (var tuple in apps)
            {
                try
                {
                    string appPath = Path.Combine(root, tuple.app);
                    string servicePath = Path.Combine(root, tuple.service);
                    string applicationManifestPath = Path.Combine(appPath, "ApplicationPackageRoot", "ApplicationManifest.xml");
                    if (!File.Exists(applicationManifestPath)) continue;
                    if (!TryParseApplicationManifest(applicationManifestPath, out var manifest)) continue;
                    bool alreadyProvisioned = false;
                    try
                    {
                        var currentTypes = await fabricClient.QueryManager.GetApplicationTypeListAsync();
                        alreadyProvisioned = currentTypes.Any(t =>
                            t.ApplicationTypeName.Equals(manifest.ApplicationTypeName, StringComparison.OrdinalIgnoreCase) &&
                            t.ApplicationTypeVersion.Equals(manifest.ApplicationTypeVersion, StringComparison.OrdinalIgnoreCase));
                    }
                    catch { }
                    string stagingRoot = null;
                    string imageStorePath = $"IT_{manifest.ApplicationTypeName}_{Guid.NewGuid():N}";
                    bool uploaded = false;
                    try
                    {
                        if (!alreadyProvisioned)
                        {
                            stagingRoot = PrepareSolutionApplicationPackage(appPath, servicePath, manifest);
                            if (stagingRoot == null) continue;
                            ...
                            provisionedApplicationTypes.Add((manifest.ApplicationTypeName, manifest.ApplicationTypeVersion));
                        }
                        string instanceName = $"IT{manifest.ApplicationTypeName}Tests";
                        var appUri = new Uri($"fabric:/{instanceName}");
                        var existing = await fabricClient.QueryManager.GetApplicationListAsync(appUri);
                        if (!existing.Any())
                        {
                            await fabricClient.ApplicationManager.CreateApplicationAsync(
                                new ApplicationDescription(appUri, manifest.ApplicationTypeName, manifest.ApplicationTypeVersion));
                            await Task.Delay(300);
                        }
                        if (!deployedApplications.Contains(instanceName))
                    ...
                    finally
                    {
                        if (uploaded)
                        {
                            try
                            {
                                fabricClient.ApplicationManager.RemoveApplicationPackage(imageStoreConnection, imageStorePath);
                            }
                            catch { }
                        }
        ...
        private string PrepareSolutionApplicationPackage(string appProjectPath, string serviceProjectPath, ApplicationManifestInfo manifest)
        {
            string staging = Path.Combine(Path.GetTempPath(), $"ITPkg_{manifest.ApplicationTypeName}_{Guid.NewGuid():N}");
            try
            {
                Directory.CreateDirectory(staging);
                File.Copy(
                    Path.Combine(appProjectPath, "ApplicationPackageRoot", "ApplicationManifest.xml"),
                    Path.Combine(staging, "ApplicationManifest.xml"));
                foreach (var svc in manifest.ServiceManifests)
                {
                    string sourceRoot = Path.Combine(serviceProjectPath, "PackageRoot");
                    if (!Directory.Exists(sourceRoot)) return null;
                    string target = Path.Combine(staging, svc.ServiceManifestName);
                    CopyDirectory(sourceRoot, target);
                    string codeDir = Path.Combine(target, "Code");
                    if (Directory.Exists(codeDir)) Directory.Delete(codeDir, true);
                    Directory.CreateDirectory(codeDir);
                    string buildOutput = TryLocateLatestBuildOutput(serviceProjectPath);
                    if (buildOutput == null) return null;
                    foreach (var f in Directory.GetFiles(buildOutput))
                    {
                        var name = Path.GetFileName(f);
                        if (name.EndsWith(".pdb", StringComparison.OrdinalIgnoreCase)) continue;
                        File.Copy(f, Path.Combine(codeDir, name), true);
                    }
                }
                return staging;
            }
        ...
        private static string TryLocateLatestBuildOutput(string serviceProjectPath)
        {
            try
            {
                string bin = Path.Combine(serviceProjectPath, "bin");
        ...
        private static bool TryParseApplicationManifest(string path, out ApplicationManifestInfo info)
        {
            info = null;
            try
            {
                ...
                var imports = root.Elements(ns + "ServiceManifestImport")
                    ...
                    .ToList();
                info = new ApplicationManifestInfo
                {
                    ApplicationTypeName = typeName,
                    ApplicationTypeVersion = typeVersion,
                    ServiceManifests = imports
                };
                return true;
            }
            catch
        ...
        private class ApplicationManifestInfo
        {
            ...
            public List<ServiceManifestRefInfo> ServiceManifests { get; set; } = new List<ServiceManifestRefInfo>();
        }

        private class ServiceManifestRefInfo
        {
            public string ServiceManifestName { get; set; }
            public string ServiceManifestVersion { get; set; }
        }

        public async Task RemoveAllDeployedApplicationsAsync(bool unprovision = false)
        {
            cleanupInvoked = true;
            if (!IsClusterAvailable) return;
            foreach (var app in deployedApplications.ToList())
            {
                try
                {
                    await RemoveApplicationAsync(app);
                    deployedApplications.Remove(app);
                }
                catch { }
            }
            if (unprovision)
            {
                foreach (var t in provisionedApplicationTypes.ToList())
                {
                    try
                    {
                        await fabricClient.ApplicationManager.UnprovisionApplicationAsync(
                            new UnprovisionApplicationTypeDescription(t.TypeName, t.TypeVersion));
                        provisionedApplicationTypes.Remove(t);
                    }
                    catch { }
                }
            }
        ...
        private async Task RemoveApplicationAsync(string app)
        {
            try
            {
                var uri = new Uri($"fabric:/{app}");
                await fabricClient.ApplicationManager.DeleteApplicationAsync(
                    new DeleteApplicationDescription(uri) { ForceDelete = true });
            }
            catch { }
        }
        ...
        private async Task<string> GetImageStoreConnectionStringAsync()
        {
            ...
            try
            {
                ...
                var m = Regex.Match(manifest, "Parameter\\s+Name=\"ImageStoreConnectionString\"\\s+Value=\"([^\"]+)\"", RegexOptions.IgnoreCase);
                if (m.Success)
                {
                    imageStoreConnectionString = m.Groups[1].Value.Trim();
                }
            }
            catch { }
        ...
        public void Dispose()
        {
            ...
            try
            {
                if (!cleanupInvoked)
                {
                    try
                    {
                        RemoveAllDeployedApplicationsAsync(false).GetAwaiter().GetResult();
                    }
                    catch { }
                }
            }
